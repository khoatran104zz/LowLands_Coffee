...
export default function ManagerOrdersPage() {
  ...
  const columns: Column<OrderExtended>[] = [
    ...
    {
      key: "totalAmount",
      header: t("admin.ordersPage.colTotal") || "Thanh tien",
      render: (item) => <span className="font-extrabold text-amber-900">{item.totalAmount.toLocaleString("vi-VN")}??</span>
    },
    {
      key: "paymentMethod",
        ...
        {selectedOrder && (
          <div className="space-y-5 text-left select-none">
                      ...
                      <React.Fragment key={idx}>
                        <tr className="hover:bg-zinc-50/50">
                          <td className="p-3">
                            <div>
                              ...
                              <span className="text-[10px] text-zinc-500 block mt-0.5">Size {item.size}</span>
                              {item.note && <span className="text-[10px] text-amber-900 block mt-0.5">Luu y: {item.note}</span>}
                            </div>
                          </td>
                          <td className="p-3 text-center">{item.quantity}</td>
                          <td className="p-3 text-right">{item.unitPrice.toLocaleString("vi-VN")}??</td>
                          <td className="p-3 text-right">{item.totalPrice.toLocaleString("vi-VN")}??</td>
                        </tr>
                        {item.toppings && item.toppings.map((top, tIdx) => (
                          <tr key={`t-${idx}-${tIdx}`} className="bg-muted/10 text-muted-foreground">
                            ...
                            <td className="py-1 px-4 text-right">{top.unitPrice.toLocaleString("vi-VN")}??</td>
                            <td className="py-1 px-4 text-right">
                              {((top.unitPrice * top.quantity) * item.quantity).toLocaleString("vi-VN")}??
                            </td>
                          </tr>
                        ))}
                      </React.Fragment>
            ...
            <div className="grid grid-cols-2 gap-4 text-xs bg-zinc-50/50 p-4 rounded-xl border border-zinc-200">
              <div>
                <span className="block text-zinc-400 font-bold uppercase text-[9px]">Ten nguoi nhan</span>
                <span className="font-semibold text-zinc-800 mt-0.5 block">{selectedOrder.receiverName}</span>
              </div>
              <div>
                <span className="block text-zinc-400 font-bold uppercase text-[9px]">Dien thoai</span>
                <span className="font-semibold text-zinc-800 mt-0.5 block">{selectedOrder.receiverPhone}</span>
              </div>
              <div className="col-span-2">
                <span className="block text-zinc-400 font-bold uppercase text-[9px]">Dia chi giao hang</span>
                <span className="font-semibold text-zinc-800 mt-0.5 block">{selectedOrder.deliveryAddress || "Mua mang di tai quay"}</span>
              </div>
            </div>
