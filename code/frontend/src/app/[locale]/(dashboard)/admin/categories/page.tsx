...
export default function AdminCategoriesPage() {
  ...
  const handleSaveCategory = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCategory) {
        if (editingCategory.status === "active" && formStatus === "inactive") {
          const isConfirmed = await confirm({
            title: t("common.confirmDeactivateTitle") || "Xác nhận ngưng hoạt động",
            message: `${t("common.confirmDeactivateMessage") || "Bạn có chắc chắn muốn ngưng hoạt động mục này?"} ("${editingCategory.name}")`,
